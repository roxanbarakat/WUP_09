...
### Best Two regressor model
```{r}
roach_two_element_model <- lm(Weight~Length3,data=Roach)
summary(roach_two_element_model)
roach_five_element_model <- lm(Weight~Length1+Length2+Length3, data=Roach)
```
...
```
By using the anova test, we can conclude that 
